...
Функция ПолучитьРольПользователя()Экспорт
	Если РольДоступна("ПолныеПрава") Тогда
		Возврат ""
	//ИначеЕсли РольДоступна("РуководительПроектов") Тогда
	//	Возврат "РабочийСтолРуководительПроектов"
	//ИначеЕсли РольДоступна("Сотрудник") Тогда
	//	Возврат "РабочийСтолСотрудника";
	//ИначеЕсли РольДоступна("РуководительПодразделения") Тогда
	//	Возврат "РабочийСтолРуководительПодразделения";
	КонецЕсли;	
КонецФункции
...
Функция ПолучитьСписокДоступныхПроектов() Экспорт
	СписокПроектов = Новый СписокЗначений;
	Запрос = Новый Запрос("ВЫБРАТЬ
	|	РуководителиПроектов.Проект КАК Проект
	|ИЗ
	|	РегистрСведений.РуководителиПроектов КАК РуководителиПроектов
	|ГДЕ
	|	РуководителиПроектов.Пользователь = &ТекущийПользователь");
	...
	Выборка = Результат.Выбрать();
	Пока Выборка.Следующий() Цикл
		СписокПроектов.Добавить(Выборка.Проект);
	КонецЦикла;
	Возврат СписокПроектов
КонецФункции	
...
Процедура СообщитьИЗаписатьВЖурнал(ИмяСобытия, ТекстСообщения, ВЖурнал = Истина, Сообщить = Истина, Знач УровеньЖурнала = Неопределено, Поле = Неопределено, ПутьКДанным = Неопределено) Экспорт
...
Процедура СообщитьИЗаписатьВЖурналПоШаблону(ИмяСобытия, ШаблонСообщения, ПараметрыСообщения = Неопределено, ВЖурнал = Истина, Сообщить = Истина, Знач УровеньЖурнала = Неопределено) Экспорт
	...
	Если ТипЗнч(ПараметрыСообщения) = Тип("Структура") Тогда
		Поле		= ПолучитьПолеСтруктуры(ПараметрыСообщения, "Сообщение_Поле");
		ПутьКДанным	= ПолучитьПолеСтруктуры(ПараметрыСообщения, "Сообщение_ПутьКДанным");
	КонецЕсли;
	СообщитьИЗаписатьВЖурнал(ТекстИмяСобытия, ТекстСообщения, ВЖурнал, Сообщить, УровеньЖурнала, Поле, ПутьКДанным);
КонецПроцедуры
