...
Процедура ПолучениеИОтправкаЭлектронныхПисем() Экспорт
	...
	Пока ВыборкаУчетныеЗаписи.Следующий() Цикл
		...
		Пока ВыборкаПисьма.Следующий() Цикл
			...
			Если ТребуетсяПереустановкаСоединения ИЛИ Соединение = Неопределено Тогда
				...
				Если Соединение <> Неопределено Тогда
					...
					Исключение
						...
						ТекстСообщенияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("ru = 'Соединение.Отключиться:
			...
			Иначе
				...
				Попытка
					...
					Если ПротоколПодключения = ПротоколИнтернетПочты.IMAP
						// {Рарус_shav 2019.03.29 83832
						//И Не СерверИсходящейПочтыНеТребуетДополнительнойОтправкиПоIMAP(ВыборкаПисьма.СерверИсходящейПочты) Тогда
						И Не СерверИсходящейПочтыНеТребуетДополнительнойОтправкиПоIMAP(ВыборкаУчетныеЗаписи.УчетнаяЗапись.СерверИсходящейПочты) Тогда
						// }Рарус_shav 2019.03.29 83832
	...
	Если ИдЗагружать.Количество() <> 0 Тогда
		...
		Пока ИдЗагружать.Количество() > (Получено + КоличествоОшибокПриЗаписи + КоличествоНеАктуальныхСообщений) Цикл
			...
			Исключение
				...
				ТекстСообщенияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						                      НСтр("ru = 'При получении письма с идентификатором %1 произошла ошибка
						                      |%2'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
						                      ?(МассивИдентификаторовПорцииДляЗагрузки.Количество(), МассивИдентификаторовПорцииДляЗагрузки[0], Неопределено),
						                      ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Процедура ПолучитьПочтуПоПроколуPOP3(ДанныеУчетнойЗаписи, Почта, Получено, СтруктураМассивовСозданныхПисем)
	...
	Исключение
		...
		ТекстСообщенияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
										НСтр("ru = 'При удалении писем произошла ошибка
										|%1'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
										ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Функция ЗаписатьЭлектронноеПисьмо(ДанныеУчетнойЗаписи,
	...
	Если ТипЗнч(Предмет) = Тип("ДокументСсылка.Обращение") Тогда
		ВзаимодействияУСП.ОбработатьПолучениеПисьмаПоОбращению(Письмо.Ссылка, Предмет, Ложь);
	КонецЕсли;
...
Процедура ЗаполнитьДокументЭлектронноеПисьмо(Письмо, Сообщение, ЭтоИсходящееПисьмо)
	...
	Если Не ЭтоИсходящееПисьмо Тогда
		...
		Письмо.ДатаПолучения = УниверсальноеВремя(Сообщение.ДатаПолучения);
		Письмо.ОтправительАдрес = ОтправительАдрес; 
		
	Иначе
		
		Письмо.СтатусПисьма = Перечисления.СтатусыИсходящегоЭлектронногоПисьма.Отправлено;
		Письмо.ДатаОтправления = УниверсальноеВремя(Сообщение.ДатаОтправления);
	...
	Письмо.Дата = УниверсальноеВремя(?(Сообщение.ДатаОтправления = Дата(1,1,1), ТекущаяДата(), Сообщение.ДатаОтправления));
...
Процедура СоздатьПредопределеннуюПапкуЭлектронныхПисем(Наименование,Владелец)
	...
	Папка.СоздаватьОбращения = Истина;
	Папка.Записать();
