	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			Исключение
				...
				ИмяФайлаГУИД = XMLСтрока(Новый УникальныйИдентификатор);
				Расширение = УдалитьНедопустимыеСимволыИзИмениФайла(Расширение);
				Если ПустаяСтрока(НомерВерсии) Тогда
					ИмяФайла = ИмяФайлаГУИД + "." + Расширение;
				Иначе
					ИмяФайла = ИмяФайлаГУИД + "." + НомерВерсии + "." + Расширение;
				КонецЕсли;
				...
				ПолноеИмяФайлаСПутем = ПутьКТому + ИмяФайла;
...
Процедура ПриЗаполненииВидовОграниченийПравОбъектовМетаданных(Описание) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.БизнесПроцессыИЗадачи") Тогда
		Описание = Описание + "
			|Справочник.ВерсииФайлов.Чтение.Объект.БизнесПроцесс.Поручение
			|Справочник.ВерсииФайлов.Изменение.Объект.БизнесПроцесс.Поручение
			|Справочник.Файлы.Чтение.Объект.БизнесПроцесс.Поручение
			|Справочник.Файлы.Изменение.Объект.БизнесПроцесс.Поручение
			|";
	КонецЕсли;
...
Функция ВсеПодчиненныеФайлы(Знач ВладелецФайлов) Экспорт
	...
	Для каждого КлючИЗначение Из ИменаСправочников Цикл
		...
		|	&ИмяСправочника КАК ПрисоединенныеФайлы
		|ГДЕ
		|	ПрисоединенныеФайлы.ВладелецФайла = &ВладелецФайлов И НЕ ПрисоединенныеФайлы.ПометкаУдаления";
...
Функция УдалитьНедопустимыеСимволыИзИмениФайла(ИмяФайла)
	...
	Для Счетчик = 1 По 30 Цикл
		ИмяФайла = СтрЗаменить(ИмяФайла, Символ(Счетчик), "");
	КонецЦикла;
	Возврат ИмяФайла;
