...
Функция ПолучитьЗначениеРеквизитаСсылочногоТипа (Реквизит, ЗначениеСтрока,СтруктураПараметровОбъекта = Неопределено)
	...
	Если Найти(ИмяОбъектаМетаданных, "Справочник") <> 0 Тогда
		...
		Если Реквизит = Метаданные.Справочники.ПользователиСервисов.ТабличныеЧасти.КонтактнаяИнформация.Реквизиты.Вид И ОбъектМенеджер = Справочники.ВидыКонтактнойИнформации Тогда
		
			Если Найти(ЗначениеСтрока, "Мобильный") > 0 Или Найти(ЗначениеСтрока, "мобильный") > 0 Тогда  
				ЗначениеРеквизита = Справочники.ВидыКонтактнойИнформации.ТелефонПользователяСервиса;	
			ИначеЕсли Найти(ЗначениеСтрока, "Телефон") > 0 Или Найти(ЗначениеСтрока, "телефон") > 0 Тогда			
				ЗначениеРеквизита = Справочники.ВидыКонтактнойИнформации.ДополнительныйТелефонПользователяСервиса;			
			ИначеЕсли Найти(ЗначениеСтрока, "Почта") > 0 Или Найти(ЗначениеСтрока, "почта") > 0 Тогда			
				ЗначениеРеквизита = Справочники.ВидыКонтактнойИнформации.EmailПользователяСервиса;			
			КонецЕсли;			
        ...
        ИначеЕсли ЗначениеЗаполнено(ЗначениеСтрока) Тогда
            ...
            Если (ИмяОбъекта = "Абоненты" Или ИмяОбъекта = "ПользователиСервисов")
...
Процедура ЗаполнитьСтруктуруПараметровОбъекта(ЧтениеXML,СтруктураПараметровОбъекта, ПолучитьТолькоОсновныеРеквизиты = Ложь) 
	...
	Если Не ПолучитьТолькоОсновныеРеквизиты И ИмяВидаОбъектаВФайле = "КонтактныеЛица" Тогда		
		СтруктураПараметровОбъекта.Вставить("Эталон",СтруктураПараметровОбъекта.Ссылка);
    ИначеЕсли Не ПолучитьТолькоОсновныеРеквизиты И ИмяВидаОбъектаВФайле = "Контрагенты" Тогда
...
Процедура ОбработатьТаблицуРеквизитовТЧОбъекта(ТЗ,ИмяТЧ,МетаданныеОбъекта,МенеджерОбъекта)
    ...
    Если ИмяТЧ = "КонтактнаяИнформация" И МенеджерОбъекта = Справочники.Абоненты Тогда
        ТЗ = Неопределено;
		Возврат;
	КонецЕсли;
...
Процедура ПрочитатьОбъект(ЧтениеXML, МенеджерОбъекта, Отправитель, Комментарий, ДополнительныеПараметры, ТзЗагруженныхОбъектовАбоненты=Неопределено) Экспорт
	...
	ИначеЕсли МетаданныеОбъекта.Имя = "ПользователиСервисов" Тогда
		
		Если лОбъект.ЭтоНовый() Тогда
						
			Если лОбъект.Партнер.Пустая() Тогда 			
				Возврат;				
			ИначеЕсли лОбъект.Партнер.ПолучитьОбъект() = Неопределено Тогда
				Возврат;
			КонецЕсли;
	...
	ИначеЕсли МетаданныеОбъекта.Имя = "ПользователиСервисов" Тогда	
		...
		Если лОбъект.Партнер.Пустая() Тогда
		    УдалитьПользователяСервисаУАбонентов(лОбъект.Ссылка);
		Иначе 			
			ДобавитьПользователяСервисаКАбонентам(лОбъект.Партнер, лОбъект.Ссылка, ДополнительныеПараметры);
		КонецЕсли;		
...
Функция ДобавитьПользователяСервисаКАбонентам(Партнер, ПользовательСервиса, ДополнительныеПараметры)
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		НаборЗаписей.Отбор.Абонент.Установить(ВыборкаДетальныеЗаписи.Ссылка);	 
				
		НоваяСтрока = НаборЗаписей.Добавить();
		НоваяСтрока.ПользовательСервиса = ПользовательСервиса;
		НоваяСтрока.Абонент = ВыборкаДетальныеЗаписи.Ссылка;
		
		ДополнительныеПараметры.Свойство("Сервис", НоваяСтрока.Сервис);		
...
Функция УдалитьПользователяСервисаУАбонентов(ПользовательСервиса)
