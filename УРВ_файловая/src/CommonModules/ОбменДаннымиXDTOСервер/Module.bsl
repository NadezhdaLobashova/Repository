...
Функция СтруктураОбъектаXDTOВДанныеИБ(КомпонентыОбмена, ДанныеXDTO, ПравилоКонвертации, Действие = "КонвертироватьИЗаписать") Экспорт
	...
	Если ЗаписыватьОбъектВИБ Тогда
		...
		ИначеЕсли ДанныеДляЗаписиВИБ <> Неопределено Тогда
...
Функция СравнитьВерсии(Знач СтрокаВерсии1, Знач СтрокаВерсии2)
	...
	Версия1 = СтрРазделить(Строка1, ".", Ложь);
	Если Версия1.Количество() < 2 ИЛИ Версия1.Количество() > 4 Тогда
	...
	КонецЕсли;
	Версия2 = СтрРазделить(Строка2, ".", Ложь);
	Если Версия2.Количество() < 2 ИЛИ Версия2.Количество() > 4 Тогда
	...
	// В версии может быть 4 разряда и в последних 2х разрядах может быть 1 и 1 - это минимальная версия, не совместимая ни с какой другой.
    // в новых версиях только 2 разряда.
	Если Версия1.Количество() = 4 И СокрЛП(Версия1[2]) = "1" И СокрЛП(Версия1[3]) = "1" Тогда
		Возврат -1;
	ИначеЕсли Версия2.Количество() = 4 И СокрЛП(Версия2[2]) = "1" И  СокрЛП(Версия2[3]) = "1" Тогда
		Возврат 1;
	КонецЕсли;
