...
Процедура ЗагрузитьФайлОбмена(ЧтениеXML,Отправитель,КоличествоПолученныхОбъектов,Комментарий)
	...
	Пока Истина Цикл
        ...
        Если Не (ЧтениеXML.Имя = "Контрагенты"  // Это не нужные объекты
                или ЧтениеXML.Имя = "Партнеры"
				или ЧтениеXML.Имя = "КонтактныеЛица") Тогда 
    ...
    Для каждого СтрТЗ Из ТзЗагруженныхОбъектовАбоненты Цикл
		...
		Если ТипЗнч(СтрТЗ.Объект) = Тип("СправочникОбъект.Абоненты") Тогда
			
			ЭлементОбъект = СтрТЗ.Объект;
			
			////!!! Фильтр абонентов по принадлезности к филиалу, еслинет партнера, то значит этот абонент не НСК.!!!
			
			Если ЭлементОбъект.Партнер.Пустая() Тогда 
				Если Не ЭлементОбъект.ЭтоНовый() Тогда		
					Продолжить;		
				КонецЕсли;
			ИначеЕсли ЭлементОбъект.Партнер.ПолучитьОбъект() = Неопределено Тогда
				
				НаборЗаписей = РегистрыСведений.БуферОбменаКонтрагентовНСИ.СоздатьНаборЗаписей();
				НаборЗаписей.Отбор.ПартнерGUID.Установить(Строка(ЭлементОбъект.Партнер.УникальныйИдентификатор()));
				...
				НоваяСтрока.ПартнерGUID = Строка(ЭлементОбъект.Партнер.УникальныйИдентификатор());
				ЗаполнитьЗначенияСвойств(НоваяСтрока, ЭлементОбъект);
				...
				Если ЭлементОбъект.ЭтоНовый() Тогда		
					Продолжить;		
				КонецЕсли;
			...
			Попытка
				...
				ОбменДаннымиНСИ_20Переопределяемый.ПриЗаписиОбъекта(ЭлементОбъект, СтруктураПараметровОбъекта);
				ЗафиксироватьТранзакцию();
			Исключение
				ОтменитьТранзакцию();
				ЗаписьВЛог ="Не удалось записать объект: "+ЭлементОбъект+" ("+СтруктураПараметровОбъекта.Ссылка+"). По причине:"+Символы.ПС+ОписаниеОшибки();
				//ОбменДаннымиНСИ_20Сервер.ЗафиксироватьОписаниеОшибки(Комментарий, ЗаписьВЛог);
				ВызватьИсключение ЗаписьВЛог;
			КонецПопытки;
			
			ОбменДаннымиНСИ_20Переопределяемый.ПослеЗаписиОбъекта(ЭлементОбъект, СтруктураПараметровОбъекта);

		//ИначеЕсли ТипЗнч(СтрТЗ.Объект) = Тип("СправочникОбъект.ПользователиСервисов") Тогда
		//	
		//	//ЭлементОбъект = СтрТЗ.Объект;
		//	
		//	МассивПользователейСервиса = НайтиПользователяСервисаПоПартнеру(СтрТЗ);
		//	
		//	Для каждого Элемент Из МассивПользователейСервиса Цикл
		//		
	...
	Если МассивПартнеров.Количество() Тогда	
		Результат = ЗарегистриоватьИзмененияДляУзла(МассивПартнеров, "Партнеры", "57"); //Новосибирск	
	КонецЕсли;
	
	//Если МассивКонтактныхЛиц.Количество() Тогда	
	//	Результат = ОбменДаннымиНСИ_20Сервер.ЗарегистриоватьИзмененияДляУзла(МассивКонтактныхЛиц, "КонтактныеЛица", "57"); //Новосибирск	
	//КонецЕсли;
...
Функция НайтиПользователяСервисаПоПартнеру(СтруктураПараметровОбъекта)
	
	//Если СтрТЗ.Объект = спр Тогда
	//
	//Иначе 
	//	
	//
	//КонецЕсли;
	//СтруктураПараметровОбъекта
	GUID = Новый УникальныйИдентификатор(СтруктураПараметровОбъекта.ПартнерВладелец);
	СсылкаПартнер = Справочники.Партнеры.ПолучитьСсылку(GUID);
	...
	Запрос.Текст = 
		...
		|ГДЕ
		|	ПользователиАбонентов.Абонент.Партнер = &Партнер";
	
	Запрос.УстановитьПараметр("Партнер", СсылкаПартнер);
	
	РезультатЗапроса = Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("ПользовательСервиса");
