...
Процедура ОпределитьМенеджерОбъектаПереопределяемый(МенеджерОбъекта,Имя,СтандартнаяОбработка) Экспорт
	...
	ИначеЕсли Имя = "КонтактныеЛица" Тогда //Грузить КонтактныеЛица в ПользователиСервисов
        СтандартнаяОбработка = Ложь;
		МенеджерОбъекта = Справочники.ПользователиСервисов;
    КонецЕсли;
...
Процедура ОбработатьТаблицуРеквизитовТЧПереопределяемый(ИмяТЧ,ТаблицаРеквизитов,Объект,СтандартнаяОбработка) Экспорт
	...
	ИначеЕсли ТипЗнч(Объект) = Тип("СправочникОбъект.ПользователиСервисов") Тогда 
		
		Если ИмяТЧ = "КонтактнаяИнформация" Тогда
			
			МассивУдаляемыхСтрок = Новый Массив;
			СтрокаМобильныйТелефон = Неопределено;
			СтрокаТелефон = Неопределено;
			СтрокаАдресЭлектроннойПочты = Неопределено;
			
			Для каждого СтрокаТаблицаРеквизитов Из ТаблицаРеквизитов Цикл
				
				Если СтрокаТаблицаРеквизитов.Тип = "АдресЭлектроннойПочты" И СтрокаАдресЭлектроннойПочты = Неопределено Тогда			
					СтрокаТаблицаРеквизитов.Вид = Справочники.ВидыКонтактнойИнформации.EmailПользователяСервиса; 	
					СтрокаАдресЭлектроннойПочты = СтрокаТаблицаРеквизитов;
				ИначеЕсли Не СтрокаТаблицаРеквизитов.Тип = "АдресЭлектроннойПочты" И Не СтрокаАдресЭлектроннойПочты = Неопределено Тогда 
					МассивУдаляемыхСтрок.Добавить(СтрокаТаблицаРеквизитов);
				ИначеЕсли СтрокаТаблицаРеквизитов.Тип = "Телефон" Тогда
					
					Если СтрокаМобильныйТелефон = Неопределено 
						И (СтрНайти(СтрокаТаблицаРеквизитов.ЗначенияПолей, "Мобильный") > 0 
						Или СтрНайти(СтрокаТаблицаРеквизитов.ЗначенияПолей, "мобильный") > 0)
						Или (СтрНайти(СтрокаТаблицаРеквизитов.Вид, "Мобильный") > 0 
						Или СтрНайти(СтрокаТаблицаРеквизитов.Вид, "мобильный") > 0) Тогда					
						СтрокаМобильныйТелефон = СтрокаТаблицаРеквизитов; 					
					ИначеЕсли СтрокаТелефон = Неопределено 
						И (СтрНайти(СтрокаТаблицаРеквизитов.ЗначенияПолей, "Телефон") > 0 
						Или СтрНайти(СтрокаТаблицаРеквизитов.ЗначенияПолей, "телефон") > 0) 
						Или (СтрНайти(СтрокаТаблицаРеквизитов.Вид, "телефон") > 0 
						Или СтрНайти(СтрокаТаблицаРеквизитов.Вид, "телефон") > 0) Тогда 											
						СтрокаТелефон = СтрокаТаблицаРеквизитов;						
					Иначе						
						МассивУдаляемыхСтрок.Добавить(СтрокаТаблицаРеквизитов);
					КонецЕсли;
			...
			Для каждого ЭлементМассивУдаляемыхСтрок Из МассивУдаляемыхСтрок Цикл				
				ТаблицаРеквизитов.Удалить(ЭлементМассивУдаляемыхСтрок);  			
			КонецЦикла;
...
Процедура ОбработатьСтруктуруПарметровОбъектаПереопределяемый(СтруктураПараметровОбъекта,Ссылка,Отправитель,СтандартнаяОбработка,Комментарий,ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли ТипЗнчСсылка = Тип("СправочникСсылка.ПользователиСервисов") Тогда
		
		Если Не СтруктураПараметровОбъекта.Свойство("ПартнерВладелец") Тогда		
			СтандартнаяОбработка = Ложь;	
		КонецЕсли;
...
Процедура ОбработатьРеквизитОбъектаПереопределяемый(ИмяРеквизита,ЗначениеРеквизита,Объект,СтандартнаяОбработка)Экспорт
	...
	ИначеЕсли ИмяМенеджера = "ПользователиСервисов" Тогда
		
		Если ИмяРеквизита = "ПартнерВладелец" Тогда		
			Объект.Партнер = Справочники.Партнеры.ПолучитьСсылку(Новый УникальныйИдентификатор(ЗначениеРеквизита));	
		КонецЕсли;
...
Процедура ПередЗаполнениемРеквизитовОбъекта(Объект, МетаданныеОбъекта, СтруктураПараметровОбъекта, ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(Объект) = Тип("СправочникОбъект.ФизическиеЛица") Тогда
		...
		//КонецЦикла;
	ИначеЕсли ТипЗнч(Объект) = Тип("СправочникОбъект.ПользователиСервисов") Тогда 
		Объект.Владелец = ДополнительныеПараметры.Сервис;
	КонецЕсли;
