...
Процедура ПриЗаписиВзаимодействия(ДокументОбъект, Отказ) Экспорт
	
	Если ДокументОбъект.ОбменДанными.Загрузка Тогда
		Возврат;
	КонецЕсли; 
	
	ТребуетсяОповещение = Ложь;
	
	Предмет = Взаимодействия.ПолучитьЗначениеПредмета(ДокументОбъект.Ссылка);
	
	Если ТипЗнч(Предмет) = Тип("ДокументСсылка.Обращение") Тогда
		Если ТипЗнч(ДокументОбъект.Ссылка) = Тип("ДокументСсылка.ЭлектронноеПисьмоВходящее") Тогда
			ТребуетсяОповещение = Истина;
		ИначеЕсли ТипЗнч(ДокументОбъект.Ссылка) = Тип("ДокументСсылка.ТелефонныйЗвонок") И ДокументОбъект.Входящий Тогда
			ТребуетсяОповещение = Истина;
		ИначеЕсли ТипЗнч(ДокументОбъект.Ссылка) = Тип("ДокументСсылка.КомментарийПользователя") Тогда
			ТребуетсяОповещение = Истина;
		КонецЕсли; 
	КонецЕсли;
	
	Если ТребуетсяОповещение Тогда
        РегистрыСведений.НаличиеОбновленийОбращений.ДобавитьЗапись(Предмет, Истина);
	КонецЕсли; 
