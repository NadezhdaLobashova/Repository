...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если не ПричинаОтмены	= ПредопределенноеЗначение("Справочник.ПричиныОтменыЗаписиНаЛК.Перенос") Тогда 
		ДокументОбъект = ПолучитьСобытие().ПолучитьОбъект();
		ДокументОбъект.Состояние = ПредопределенноеЗначение("Перечисление.СостоянияОбращений.Закрыто");
		ДокументОбъект.ДатаЗакрытия = ТекущаяУниверсальнаяДата();
...
Функция ПолучитьДвиженияРасписаниеКонсультаций(РежимПроведения, Отказ) Экспорт
	...
	ЭтоПривилегированныйСотрудник	= (ДанныеПоРолям["Итого"] - ДанныеПоРолям[Перечисления.РолиСотрудниковЛК.Сотрудник]) > 0 ИЛИ (ДоступностьДляПП И ДР.ПолныеПрава) ИЛИ ДР.ВебСервисHotline;
	//Лобашова 20.08.2019 91504 -
	ЭтоСотрудник					= ДанныеПоРолям[Перечисления.РолиСотрудниковЛК.Сотрудник] > 0;
	
	Если Не ЭтоПривилегированныйСотрудник Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = "Для отмены записи недостаточно прав.";
	...
	|ВЫБРАТЬ
	|	&Период,
	|	ОтменаЗаписиНаЛК.Сотрудник,
	|	ВТТекущееРасписание.Работа,
	|	ВТТекущееРасписание.Проект,
	|	ОтменаЗаписиНаЛК.ДатаВремяНачала,
	|	ВТТекущееРасписание.ДатаВремяОкончания,
	|	ВТТекущееРасписание.Продолжительность,
	|	ОтменаЗаписиНаЛК.ПричинаОтмены,
	|	ЗНАЧЕНИЕ(Перечисление.СостоянияЭлементаРасписанияЛК.Запланировано) КАК Состояние
	|ИЗ
	|	Документ.ОтменаЗаписиНаЛК КАК ОтменаЗаписиНаЛК
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТТекущееРасписание КАК ВТТекущееРасписание
	|		ПО ОтменаЗаписиНаЛК.Сотрудник = ВТТекущееРасписание.Сотрудник
	|			И ОтменаЗаписиНаЛК.ДатаВремяНачала = ВТТекущееРасписание.ДатаВремяНачала
	|ГДЕ
	|	ОтменаЗаписиНаЛК.Ссылка = &Ссылка";
