...
Процедура ПредметПриИзменении(Элемент)
	...
	Если Вложения.Количество() > 0 И ТипЗнч(Предмет) = Тип("ДокументСсылка.Обращение") И ЗначениеЗаполнено(Предмет) Тогда
		Оповещение = Новый ОписаниеОповещения("ПредметПриИзмененииЗавершение", ЭтотОбъект);
		ПоказатьВопрос(Оповещение, НСтр("ru='Скопировать файлы из письма в выбранное обращение?'"), РежимДиалогаВопрос.ДаНет, 60, КодВозвратаДиалога.Да); 
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура ПредметПриИзмененииЗавершение(Результат, ПараметрыОповещения) Экспорт
	
	Если Результат = КодВозвратаДиалога.Да Тогда 
		СкопироватьПрисоединенныеФайлыИзПисьмаВОбращение();
		ПоказатьПредупреждение(,"Файлы скопированы.", 60);
	КонецЕсли;
	
КонецПроцедуры

&НаСервере
Процедура СкопироватьПрисоединенныеФайлыИзПисьмаВОбращение() 
	
	ПрисоединенныеФайлыУСП.СкопироватьПрисоединенныеФайлыИзПисьмаВОбращение(Объект.Ссылка, Предмет);
...
Функция НовоеОбращениеНаСервере()
    ...
    ДанныеВитрин = ВзаимодействияУСП.ДанныеВитрин();
    
    Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ РАЗРЕШЕННЫЕ ПЕРВЫЕ 1
        |   Письма.Ссылка КАК Письмо,
        |   Письма.УчетнаяЗапись,
        |   Письма.Текст,
        |   Письма.ТекстHTML,
        |   Письма.ТипТекста,
        ...
        |   ),
        |   Письма.Тема,
        |   Письма.ОтправительКонтакт,
        |   Письма.ОтправительАдрес,
        |   Письма.ОтправительПредставление,
		// Лобашова 02.11.2018 75109 +
    ...
    НовоеОбращение = Документы.Обращение.ПустаяСсылка();
    ДанныеПисьма = Запрос.Выполнить().Выбрать();
    Если ДанныеПисьма.Следующий() Тогда
        НовоеОбращение = ВзаимодействияУСП.НовоеОбращениеПоПисьму(ДанныеПисьма, ДанныеВитрин, Истина);
        Если НовоеОбращение = Неопределено Тогда
            Возврат Неопределено;
        КонецЕсли;
   		ВзаимодействияУсп.ОбработатьПолучениеПисьмаПоОбращению(ДанныеПисьма.Письмо, НовоеОбращение, Истина);
    КонецЕсли;
    
    Возврат НовоеОбращение;
...
Процедура УстановитьВидимостьДляКлиентаКомпании()
	...
	Если УРВСервер.ПроверитьДоступностьРолиКлиентКомпании() Тогда
		...
		Элементы.ОтправительПредставление.КнопкаОткрытия = Ложь;		
