...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Объект.Ссылка.Пустая() Тогда
		...
		УстановитьПризнакВнутреннейПерепискиПоПредмету();
    ...
    Если ТипЗнч(Предмет) = Тип("ДокументСсылка.Обращение") Тогда
        СервисПредмета = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Предмет, "Сервис");
    КонецЕсли; 
	ДоработкаФормы();
...
Процедура КонтактНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Отбор = Новый Структура("Владелец", СервисПредмета);
	
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("Отбор", Отбор);
	
	Оповещение = Новый ОписаниеОповещения("КонтактНачалоВыбораЗавершение", ЭтотОбъект);
...
&НаКлиенте
Процедура КонтактНачалоВыбораЗавершение(Результат, ПараметрыОповещения) Экспорт
	
	Если Результат <> Неопределено Тогда
		Объект.АбонентКонтакт = Результат.ПользовательСервиса;
	КонецЕсли; 
...
&НаСервере
Процедура УстановитьПризнакВнутреннейПерепискиПоПредмету() 
	
	Если ТипЗнч(Предмет) = Тип("ДокументСсылка.Обращение") И ЗначениеЗаполнено(Предмет) Тогда
		ИнициаторПредмета = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Предмет, "Инициатор");
	Иначе
		ИнициаторПредмета = Неопределено;
	КонецЕсли;
	
	Если ИнициаторПредмета <> Объект.АбонентКонтакт Тогда
		Объект.ВнутренняяПереписка = Истина;
	Иначе
		Объект.ВнутренняяПереписка = Ложь;
	КонецЕсли;
...
Процедура ЗаполнитьПоПереданнымПараметрам()
	...
	Если Параметры.Свойство("ЗначенияЗаполнения") Тогда
		...
		Если Параметры.ЗначенияЗаполнения.Свойство("Описание") Тогда
			Объект.Описание = Параметры.ЗначенияЗаполнения.Описание;
		КонецЕсли; 
	КонецЕсли; 
...
&НаСервере
Процедура ДоработкаФормы()
	Элементы.СтраницаКомментарий.Видимость = Ложь;
	Элементы.Предмет.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Лево;
	Элементы.Ответственный.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Лево;
	Элементы.АбонентПредставление.Видимость = Ложь;
	Элементы.АбонентКакСвязаться.Видимость = Ложь;
	Элементы.Переместить(Элементы.АбонентКонтакт, Элементы.ГруппаПраво, Элементы.Предмет);
	Элементы.Переместить(Элементы.ГруппаДатаЗвонкаВходящий, Элементы.ГруппаПраво, Элементы.АбонентКонтакт);
	Элементы.ГруппаДатаЗвонкаВходящий.Группировка = ГруппировкаПодчиненныхЭлементовФормы.Горизонтальная;
КонецПроцедуры
