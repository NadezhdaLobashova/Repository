...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.Пояснение.Заголовок = СтрШаблон(НСтр("ru = 'Для сопоставления данных ""%1""
...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	Если ВРег(ИсточникВыбора.ИмяФормы) = ВРег("Обработка.СопоставлениеОбъектовXDTO.Форма.НастройкаАвтоматическогоСопоставления") Тогда
		...
		ОткрытьФорму("Обработка.СопоставлениеОбъектовXDTO.Форма.РезультатАвтоматическогоСопоставления", ПараметрыФормы, ЭтотОбъект,,,,,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
		
	ИначеЕсли ВРег(ИсточникВыбора.ИмяФормы) = ВРег("Обработка.СопоставлениеОбъектовXDTO.Форма.РезультатАвтоматическогоСопоставления") Тогда
	...
	ИначеЕсли ВРег(ИсточникВыбора.ИмяФормы) = ВРег("Обработка.СопоставлениеОбъектовXDTO.Форма.НастройкаПолейТаблицы") Тогда
	...
	ИначеЕсли ВРег(ИсточникВыбора.ИмяФормы) = ВРег("Обработка.СопоставлениеОбъектовXDTO.Форма.НастройкаПолейТаблицыСопоставления") Тогда
	...
	ИначеЕсли ВРег(ИсточникВыбора.ИмяФормы) = ВРег("Обработка.СопоставлениеОбъектовXDTO.Форма.НастройкаСортировки") Тогда
	...
	ИначеЕсли ВРег(ИсточникВыбора.ИмяФормы) = ВРег("Обработка.СопоставлениеОбъектовXDTO.Форма.ФормаВыбораСвязиСопоставления") Тогда
...
Процедура ВыполнитьАвтоматическоеСопоставление(Команда)
	...
	ОткрытьФорму("Обработка.СопоставлениеОбъектовXDTO.Форма.НастройкаАвтоматическогоСопоставления", ПараметрыФормы, ЭтотОбъект,,,,,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура ИзменитьПоляТаблицы(Команда)
	...
	ОткрытьФорму("Обработка.СопоставлениеОбъектовXDTO.Форма.НастройкаПолейТаблицы", ПараметрыФормы, ЭтотОбъект,,,,,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура НастройкаСпискаПолейТаблицы(Команда)
	...
	ОткрытьФорму("Обработка.СопоставлениеОбъектовXDTO.Форма.НастройкаПолейТаблицыСопоставления", ПараметрыФормы, ЭтотОбъект,,,,,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура Сортировка(Команда)
	...
	ОткрытьФорму("Обработка.СопоставлениеОбъектовXDTO.Форма.НастройкаСортировки", ПараметрыФормы, ЭтотОбъект,,,,,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура ОтменитьУтвержденноеСопоставлениеНаСервере(Отбор)
	...
	Если ОбменДаннымиXDTOПовтИсп.ЭтоПланОбменаXDTO(Объект.УзелИнформационнойБазы) Тогда
		Отбор.Вставить("Ссылка", Отбор.УникальныйИдентификаторИсточника);
		Отбор.Вставить("Идентификатор", Отбор.УникальныйИдентификаторПриемника);
	КонецЕсли;
...
Процедура ВыполнитьПроверкуЗаданияПользовательскихПолей(Отказ, ПользовательскиеПоля)
	...
	ИначеЕсли ПользовательскиеПоля.Количество() > МаксимальноеКоличествоПользовательскихПолей() Тогда
		...
		СтрокаСообщения = СтрШаблон(СтрокаСообщения, Строка(МаксимальноеКоличествоПользовательскихПолей()));
...
Процедура УстановитьСвязьИнтерактивно()
	...
	Если НельзяУстановитьСвязьБыстро Тогда
		...
		ОткрытьФорму("Обработка.СопоставлениеОбъектовXDTO.Форма.ФормаВыбораСвязиСопоставления", ПараметрыФормы, ЭтотОбъект,,,,,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура ВыполнитьСопоставлениеОбъектов(Отказ)
	...
	Попытка
		...
		Если ОбщегоНазначения.ИнформационнаяБазаФайловая() Тогда
			...
			РезультатСопоставления = Обработки.СопоставлениеОбъектовXDTO.РезультатСопоставленияОбъектов(ПараметрыМетода);
		...
		Иначе
			...
			Результат = ДлительныеОперации.ЗапуститьВыполнениеВФоне(
				...
				"Обработки.СопоставлениеОбъектовXDTO.ВыполнитьСопоставлениеОбъектов",
				ПараметрыМетода,
...
Процедура ПослеСопоставленияОбъектов(Знач РезультатСопоставления)
	...
	ОбработкаОбъект = Обработки.СопоставлениеОбъектовXDTO.Создать();
