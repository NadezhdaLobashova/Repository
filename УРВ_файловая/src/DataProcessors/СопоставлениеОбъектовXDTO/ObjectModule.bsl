...
Процедура ПрименитьТаблицуНеутвержденныхЗаписей(Отказ) Экспорт
	...
	Попытка
		...
		Для Каждого СтрокаТаблицы Из ТаблицаНеутвержденныхСвязей Цикл
			...
			Если ОбменДаннымиXDTOПовтИсп.ЭтоПланОбменаXDTO(УзелИнформационнойБазы) Тогда
...
Процедура ВыполнитьЗагрузкуДанныхВИнформационнуюБазу(Отказ, ТаблицыДляЗагрузки) Экспорт
	...
	Если ОбработкаОбменаДанными.ФлагОшибки() Тогда
		...
		НСтрока = СтрШаблон(НСтрока, ОбработкаОбменаДанными.СтрокаСообщенияОбОшибке());
...
Процедура Конструктор() Экспорт
	...
	СписокПолейТаблицы.ЗагрузитьЗначения(СтрРазделить(ПоляТаблицыПриемника,","));
	
	МассивПолейПоиска = СтрРазделить(ПоляПоискаТаблицыПриемника, ",");
...
Процедура ЗаполнитьСписокДополнительнымиПараметрами(СписокПолейТаблицы)
	...
	Для каждого Элемент Из СписокПолейТаблицы Цикл
		...
		Если Реквизит = Неопределено Тогда
			ВызватьИсключение СтрШаблон(НСтр("ru = 'Для объекта метаданных ""%1"" не определен реквизит с именем ""%2""'"),
		...
		Если ЭтоСтрокаНеограниченнойДлины(Реквизит) Тогда
			...
			Представление = СтрШаблон("%1 %2", ?(ПустаяСтрока(Реквизит.Синоним), Реквизит.Имя, СокрЛП(Реквизит.Синоним)),
