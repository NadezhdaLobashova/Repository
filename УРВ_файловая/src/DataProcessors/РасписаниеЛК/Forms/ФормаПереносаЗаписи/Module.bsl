...
Функция СоздатьДокументыНаСервере(ВыбраннаяСтрока, ТекстОшибки = Неопределено) Экспорт
	...
	ДокументОбъект.Событие				= ПолучитьОбращение(ВыбраннаяСтрока);
	//Лобашова 89291 17.07.2019 -
...
//Лобашова 89291 17.07.2019 +
Функция  ПолучитьОбращение(ВыбраннаяСтрока)
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	РасписаниеКонсультацийСрезПоследних.Сотрудник КАК Сотрудник,
	...
	Запрос.УстановитьПараметр("ДатаВремяНачала", ИсходнаяСтруктура.ДатаВремяНачала);
	Запрос.УстановитьПараметр("Сотрудник", ИсходнаяСтруктура.Сотрудник);
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		Событие.Исполнитель = ВыбраннаяСтрока.Сотрудник;
