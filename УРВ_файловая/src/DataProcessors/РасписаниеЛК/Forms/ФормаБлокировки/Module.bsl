...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ТаблицаСлотов.Количество() = 0 Тогда
		СтрокаТаблицаСлотов = ТаблицаСлотов.Добавить();
	КонецЕсли;
	ОписаниеXML = ТаблицаСлотов[0].ОписаниеXML;// в ТЧ пока не используется
	БоР_ОбщийМодульКлиентСервер.РазвернутьФорматированныйДокументИзXML(ОписаниеXML, ОписаниеФД, , );
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	БоР_ОбщийМодуль.УстановитьПроверкуЗаполненияРеквизита(ПроверяемыеРеквизиты, "Проект", НЕ Работа.Отсутствие);
...
Процедура РаботаПриИзменении(Элемент)
	...
	Если БоР_ОбщийМодуль.ПолучитьПолеОбъекта(Работа, "Отсутствие") Тогда
		Проект			= Неопределено;
	КонецЕсли;
...
Процедура УстановитьВидимостьДоступность(ЭтаФорма)
	...
	//
	БоР_ОбщийМодульКлиентСервер.НастроитьОбязательностьРеквизита(
		ЭтаФорма.Элементы.Проект, 
		НЕ БоР_ОбщийМодуль.ПолучитьПолеОбъекта(ЭтаФорма.Работа, "Отсутствие"), 
		ЭтаФорма.Проект);
