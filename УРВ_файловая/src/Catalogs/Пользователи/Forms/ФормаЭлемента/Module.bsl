...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЗаполнятьИзОБД = Параметры.Свойство("МассивURMExchange") и НЕ ЗначениеЗаполнено(Объект.Ссылка);
	Если ЗаполнятьИзОБД Тогда
		ЗаполнитьДанныеПоМассиву(Параметры.МассивURMExchange);
	КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		Если ЗначениеЗаполнено(Параметры.ЗначениеКопирования) Тогда
			...
			//astyul, 71975, 10.09.2018
			Если Не УровеньДоступа.ИзменениеРазрешенияНаВход И НЕ РольДоступна("ИТ_Служба") Тогда
			//astyul, 71975, 10.09.2018	
		...
		Иначе
			...
			Если Не ЗначениеЗаполнено(Параметры.ИдентификаторПользователяИБ) и НЕ ЗаполнятьИзОБД Тогда
				...
				КонецЕсли;
				//astyul, 71975, 10.09.2018
				Если УровеньДоступа.ИзменениеРазрешенияНаВход ИЛИ РольДоступна("ИТ_Служба") Тогда
				//astyul, 71975, 10.09.2018
	...
	Если ЗначениеЗаполнено(Параметры.ЗначениеКопирования) Тогда
		СведенияОПользователе = РегистрыСведений.СведенияОПользователях.СведенияОПользователе(Параметры.ЗначениеКопирования);
	Иначе
		СведенияОПользователе = РегистрыСведений.СведенияОПользователях.СведенияОПользователе(Объект.Ссылка);
	КонецЕсли;
    ...
    ПотребоватьСменуПароляПриВходе = СведенияОПользователе.ПотребоватьСменуПароляПриВходе;
	
	Если Не РольДоступна(Метаданные.Роли.ПолныеПрава) Тогда
		Элементы.ОбслуживающаяОрганизация.ТолькоПросмотр = Истина;
		Элементы.ЛинияПоддержки.ТолькоПросмотр = Истина;
	КонецЕсли; 
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "ИзменениеДанныхОБДПоПользователю" Тогда
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	МенеджерЗаписи.ОбслуживающаяОрганизация = ОбслуживающаяОрганизация;
	МенеджерЗаписи.ЛинияПоддержки = ЛинияПоддержки;
    МенеджерЗаписи.ПотребоватьСменуПароляПриВходе = ПотребоватьСменуПароляПриВходе;
	МенеджерЗаписи.Записать();
	
	//astyul, 71975, 10.09.2018
	Если РольДоступна("ИТ_Служба") Тогда
		ДобавитьПользователяВГруппуДоступа();
	КонецЕсли;
	//astyul, 71975, 10.09.2018
...
Процедура ОбслуживающаяОрганизацияПриИзменении(Элемент)
	...
	НельзяМенятьПоляСведений = Не ЗначениеЗаполнено(ОбслуживающаяОрганизация);
	Элементы.ЛинияПоддержки.ТолькоПросмотр = НельзяМенятьПоляСведений;
	
КонецПроцедуры

//astyul, 71975, 10.09.2018
&НаСервере
Процедура ДобавитьПользователяВГруппуДоступа()
	Запрос = Новый Запрос("ВЫБРАТЬ
	                      |	ГруппыДоступа.Ссылка
	                      |ИЗ
	                      |	Справочник.ГруппыДоступа КАК ГруппыДоступа
	                      |ГДЕ
	                      |	ГруппыДоступа.ОсновнаяГруппаДоступаПользователя");
	...
	Пока Выборка.Следующий() Цикл
		ОбъектГруппа = Выборка.Ссылка.ПолучитьОбъект();
		строкаПользователь = ОбъектГруппа.Пользователи.Добавить();
		строкаПользователь.Пользователь = Объект.Ссылка;
		Попытка
...
Процедура ЗаполнитьДанныеПоМассиву(МассивURMExchange)
	Если МассивURMExchange.Количество() = 0 Тогда
		Возврат
	КонецЕсли;
	СтруктураОтвета                                = МассивURMExchange[0];
	Объект.Наименование                            = СтруктураОтвета.Name;
	Объект.GUIDОБД                                 = СтруктураОтвета.GUIDОБД;
	ЭтотОбъект.ДоступКИнформационнойБазеРазрешен   = Истина;
	ЭтотОбъект.ПользовательИнфБазыАутентификацияОС = Истина;
	ЭтотОбъект.ПользовательИнфБазыПользовательОС   = СтруктураОтвета.authentication;
	ПользовательИнфБазыИмя                         = СтруктураОтвета.Name;
КонецПроцедуры
...
Процедура URMExchangeОбновитьДанныеНаСервере()
	...
	СписокXDTO = ОбработкаОбъект.ЗагрузкаДанныхИзURM("GetEmployee");
    строкаПроектXDTO = Неопределено;
	Для Каждого строкаXDTO Из СписокXDTO Цикл
		Если строкаXDTO.GUID = СокрЛП(Объект.GUIDОБД) Тогда
	...
	Если строкаПроектXDTO = Неопределено Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = "Пользователь не найден в ОБД!";
		Сообщение.Сообщить();
		Возврат
	КонецЕсли;
	Объект.Наименование                = строкаПроектXDTO.Name;
	ПользовательИБПользовательОС       = строкаПроектXDTO.authentication;
	ПользовательИБИмя                  = строкаПроектXDTO.Name;
