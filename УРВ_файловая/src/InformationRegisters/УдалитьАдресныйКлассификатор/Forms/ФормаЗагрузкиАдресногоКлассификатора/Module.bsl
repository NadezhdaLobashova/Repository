...
Функция ЗагрузитьКЛАДР()
	...
	Попытка
		...
		Если ИсточникДанныхДляЗагрузки = 1 Тогда
			...
			Для Каждого АдресныйОбъект Из АдресныеОбъекты Цикл
				АдресныйКлассификаторКлиент.ПередатьФайлыНаСерверПоАдреснымОбъектам(
			...
			АдресныйКлассификаторКлиент.ПередатьФайлНаСервер(ПутьКДанным, "altnames.zip", ПутьКДаннымНаСервере);
		...
		Для Каждого АдресныйОбъект Из АдресныеОбъекты Цикл
			...
			АдресныйКлассификатор.ЗагрузитьКлассификаторПоАдресномуОбъекту(	АдресныйОбъект,
		...
		УстановитьСтатусЗагрузки(НСтр("ru = 'Загружаются адресные сокращения'"));
		АдресныйКлассификатор.ЗагрузитьАдресныеСокращения(ПутьКДаннымНаСервере);
		
		Если ИсточникДанныхДляЗагрузки = 1 Тогда
			АдресныйКлассификаторКлиентСервер.ПолучитьФайлВерсийИОбновитьВерсиюАдресныхСведений(АдресныеОбъекты);
		КонецЕсли;
	...
	ОбщегоНазначения.УдалитьФайлыНаСервере1СПредприятие(ПутьКДаннымНаСервере);
