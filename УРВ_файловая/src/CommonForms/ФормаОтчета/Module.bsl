...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ВидимостьКомандВариантовОтчетов = Истина;//ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Параметры, "ВидимостьКомандВариантовОтчетов");	
...
Процедура ОтчетТабличныйДокументОбработкаРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка)
	...
	Если ЭтотОбъект.КлючОбъекта = "Отчет.ОбработкаОбращений" Тогда
		...
		#ИначеЕсли ВебКлиент Тогда
			Если Элемент.Рисунки.Количество() > 0 И Элемент.Рисунки[0].ТипРисунка = ТипРисункаТабличногоДокумента.Диаграмма Тогда
				СтандартнаяОбработка = Ложь;
				Возврат;
			КонецЕсли; 
		#КонецЕсли 
		
		СкорректироватьДанныеРасшифровки(Расшифровка, СтандартнаяОбработка);
...
Процедура БыстрыеНастройкиУдалитьСтарыеЭлементыИКоманды(ПараметрыЗаполнения)
	...
	Для Каждого Элемент Из УдаляемыеЭлементы Цикл
		...
		Исключение
			//Возможно, элемент формы создан интерактивно, почему то его нельзя удалить...
		КонецПопытки;
	КонецЦикла;
...
&НаСервере
Процедура СкорректироватьДанныеРасшифровки(Расшифровка, СтандартнаяОбработка, НеОтбираемыеРесурсы = Неопределено) Экспорт
	
	Данные = ПолучитьИзВременногоХранилища(ОтчетДанныеРасшифровки);
	
	ИсходныеДанные = ?(ПустаяСтрока(ИсходныеДанныеРасшифровки), Неопределено, 
		ПолучитьИзВременногоХранилища(ИсходныеДанныеРасшифровки));
	ИсходныеНастройки = ?(ПустаяСтрока(ИсходныеНастройкиРасшифровки), Неопределено, 
		ПолучитьИзВременногоХранилища(ИсходныеНастройкиРасшифровки));
	
	Если ИсходныеДанные <> Данные Тогда
		ИсходныеНастройки = Новый НастройкиКомпоновкиДанных;
		ФайлXML = ПолучитьИмяВременногоФайла();
		ЗаписьXML = Новый ЗаписьXML;
		ЗаписьXML.ОткрытьФайл(ФайлXML);
		СериализаторXDTO.ЗаписатьXML(ЗаписьXML, Данные.Настройки);
		ЗаписьXML.Закрыть();
		ЧтениеXML = Новый ЧтениеXML;
		ЧтениеXML.ОткрытьФайл(ФайлXML);
		ЧтениеXML.ПерейтиКСодержимому();
		ИсходныеНастройки = СериализаторXDTO.ПрочитатьXML(ЧтениеXML, Тип("НастройкиКомпоновкиДанных"));
		ИсходныеДанные = Данные;
	Иначе
		ФайлXML = ПолучитьИмяВременногоФайла();
		ЗаписьXML = Новый ЗаписьXML;
		ЗаписьXML.ОткрытьФайл(ФайлXML);
		СериализаторXDTO.ЗаписатьXML(ЗаписьXML, ИсходныеНастройки);
		ЗаписьXML.Закрыть();
		ЧтениеXML = Новый ЧтениеXML;
		ЧтениеXML.ОткрытьФайл(ФайлXML);
		ЧтениеXML.ПерейтиКСодержимому();
		Данные.Настройки = СериализаторXDTO.ПрочитатьXML(ЧтениеXML, Тип("НастройкиКомпоновкиДанных"));
		
	КонецЕсли;
	
	ИсходныеНастройкиРасшифровки = ПоместитьВоВременноеХранилище(ИсходныеНастройки, УникальныйИдентификатор);
	ИсходныеДанныеРасшифровки = ПоместитьВоВременноеХранилище(ИсходныеДанные, УникальныйИдентификатор);
	
	ПоляРасшифровки = Данные.Элементы[Расшифровка].ПолучитьПоля();
	
	ИмяРесурса = ПоляРасшифровки[0].Поле;
	
	ПолеКомпоновкиДанных = Новый ПолеКомпоновкиДанных(ИмяРесурса);
	
	Если НеОтбираемыеРесурсы = Неопределено Тогда
		НеОтбираемыеРесурсы = Новый Массив;
	КонецЕсли;
	
	НеОтбираемыеРесурсы.Добавить("Длительность");
	НеОтбираемыеРесурсы.Добавить("ДлительностьИсправление");
	НеОтбираемыеРесурсы.Добавить("ДлительностьРасследование");
	НеОтбираемыеРесурсы.Добавить("ДлительностьОжидание");
	НеОтбираемыеРесурсы.Добавить("ДлительностьНовое");
	
	Если НеОтбираемыеРесурсы.Найти(ИмяРесурса) = Неопределено Тогда
		ЭлементОтбора = Данные.Настройки.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
		ЭлементОтбора.ЛевоеЗначение = ПолеКомпоновкиДанных;
		ЭлементОтбора.ПравоеЗначение = 0;
		ЭлементОтбора.ВидСравнения = ВидСравненияКомпоновкиДанных.НеРавно;
	КонецЕсли; 
	
	РасположениеРесурсов = Данные.Настройки.ПараметрыВывода.Элементы.Найти("РасположениеРесурсов");
	Если РасположениеРесурсов <> Неопределено Тогда
		РасположениеРесурсов.Значение = РасположениеРесурсовКомпоновкиДанных.Горизонтально;
	КонецЕсли;
	
	ОбработатьПоляВыбора(Данные.Настройки.Выбор.Элементы, ПолеКомпоновкиДанных);
	
КонецПроцедуры

&НаСервере
Процедура ОбработатьПоляВыбора(ЭлементыВыбора, ПолеКомпоновкиДанных)
	
	Для каждого ЭлементВыбора Из ЭлементыВыбора Цикл 
		Если ТипЗнч(ЭлементВыбора) = Тип("ГруппаВыбранныхПолейКомпоновкиДанных") Тогда
			ОбработатьПоляВыбора(ЭлементВыбора.Элементы, ПолеКомпоновкиДанных);
		Иначе
			Если ЭлементВыбора.Поле <> ПолеКомпоновкиДанных Тогда
