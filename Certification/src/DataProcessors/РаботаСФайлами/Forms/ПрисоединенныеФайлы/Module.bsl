...
Процедура ВставитьСтрокиНаСервере(ЧислоФайлов)
	...
	Для Каждого ФайлПринятый Из МассивФайлов Цикл
		Если ТипВсеПрисоединенныеФайлы.СодержитТип(ТипЗнч(ФайлПринятый)) Тогда
			УправлениеЭлектроннойПочтой.ЗаписатьВложениеЭлектронногоПисьмаСкопировавВложениеДругогоПисьма(Параметры.ВладелецФайла, ФайлПринятый, УникальныйИдентификатор);
		ИначеЕсли ТипЗнч(ФайлПринятый) = Тип("Структура") Тогда
			УправлениеЭлектроннойПочтой.ЗаписатьВложениеЭлектронногоПисьмаИзВременногоХранилища(Параметры.ВладелецФайла, ФайлПринятый.ИмяФайлаНаКомпьютере, ФайлПринятый.ИмяФайла, ФайлПринятый.Размер);
