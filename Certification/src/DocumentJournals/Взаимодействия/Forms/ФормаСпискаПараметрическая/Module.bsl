...
&НаКлиенте
Процедура СоздатьКомментарийПользователя(Команда)
    
    СоздатьНовоеВзаимодействие("КомментарийПользователя");
...
Процедура ЗаполнитьДеревоВзаимодействий()
	...
	КомпоновщикНастроек.Настройки.ПараметрыДанных.УстановитьЗначениеПараметра("СекундДоМестногоВремени",ТекущаяДатаКлиента - УниверсальноеВремя(ТекущаяДатаКлиента));
	...
	Если ТолькоПочта Тогда
		...
		СписокТипыПрочиеВзаимодействия.Добавить(Тип("ДокументСсылка.КомментарийПользователя"));
