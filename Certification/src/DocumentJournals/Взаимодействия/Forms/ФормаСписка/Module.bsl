...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Список.Параметры.УстановитьЗначениеПараметра("Черновик", Перечисления.СтатусыИсходящегоЭлектронногоПисьма.Черновик);
	...
	Для Каждого ТипПредмета Из Метаданные.РегистрыСведений.ПредметыПапкиВзаимодействий.Ресурсы.Предмет.Тип.Типы() Цикл
		Если ТолькоПочта И ТипПредмета = Тип("ДокументСсылка.ТелефонныйЗвонок") Тогда
			Продолжить;
		КонецЕсли;
...
Процедура ЗаполнитьТаблицуКатегорий()
	...
	|		И ТИПЗНАЧЕНИЯ(ДополнительныеСведения.Объект) В (ТИП(Документ.КомментарийПользователя), ТИП(Документ.ТелефонныйЗвонок), ТИП(Документ.ЭлектронноеПисьмоВходящее), ТИП(Документ.ЭлектронноеПисьмоИсходящее))
	|				 " + ТекстУсловияСведения + "
	|	
	|	СГРУППИРОВАТЬ ПО
...
Процедура ЗаполнитьДеревоПапок()
	...
	|		ПО (ПапкиЭлектронныхПисем.Ссылка = РассчитанныеПапки.Папка)
	|ГДЕ
...
Процедура ДополнитьМенюПанелиНавигации()
	...
	|ГДЕ
	|	НаборыДополнительныхРеквизитовИСведенийДополнительныеРеквизиты.Ссылка В (ЗНАЧЕНИЕ(Справочник.НаборыДополнительныхРеквизитовИСведений.Документ_ТелефонныйЗвонок), ЗНАЧЕНИЕ(Справочник.НаборыДополнительныхРеквизитовИСведений.Документ_ЭлектронноеПисьмоВходящее))
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	|ГДЕ
	|	НаборыДополнительныхРеквизитовИСведенийДополнительныеСведения.Ссылка В (ЗНАЧЕНИЕ(Справочник.НаборыДополнительныхРеквизитовИСведений.Документ_ТелефонныйЗвонок), ЗНАЧЕНИЕ(Справочник.НаборыДополнительныхРеквизитовИСведений.Документ_ЭлектронноеПисьмоВходящее))
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
...
Функция НайтиВзаимодействияПолнотекстовыйПоиск(КоличествоЭлементов)
	...
	Если Не ТолькоПочта Тогда
		...
		ОбластьПоиска.Добавить(Метаданные.Документы.КомментарийПользователя);
		ОбластьПоиска.Добавить(Метаданные.Справочники.КомментарийПользователяПрисоединенныеФайлы);
