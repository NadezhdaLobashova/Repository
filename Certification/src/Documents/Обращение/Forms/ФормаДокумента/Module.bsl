...
#КонецЕсли
	...
	Если ДоступноСотруднику.ДоступныВсеДействия Тогда
		...
		Элементы.ПеренестиОбращение.Видимость 				= Истина;
		Элементы.ЗаписатьЕщеРаз.Видимость 					= Истина;
		Элементы.ОтменитьЗапись.Видимость					= Истина;
		элементы.НеОказанаКонсультация.Видимость 			= Истина;
...
Процедура ПроверитьСвязьСРасписаниемЛК()
	ЗаписьНаЛК = ПолучитьДанныеЗаписи();
	Если ЗаписьНаЛК = Неопределено или ЗакрытоОбращение() Тогда
...
Функция СоздатьДокументВыполнениеЛК(ОказанаКонсультация, Причина = Неопределено)
	...
	ДокументОбъект.ДатаВремяНачалаФакт		= ЗаписьНаЛК.ДатаВремяНачалаФакт;
	ДокументОбъект.ДатаВремяОкончанияФакт	= ЗаписьНаЛК.ДатаВремяОкончанияФакт;
	ДокументОбъект.ПродолжительностьФакт	= ЗаписьНаЛК.ПродолжительностьФакт;
