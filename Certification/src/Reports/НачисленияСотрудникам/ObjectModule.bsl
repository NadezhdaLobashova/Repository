...
Процедура ПриКомпоновкеРезультата(ДокументРезультат, ДанныеРасшифровки, СтандартнаяОбработка)
	...
	Попытка
		...
		НастройкиОтчета.ПараметрыДанных.УстановитьЗначениеПараметра("Подразделение", ПараметрыСеанса.ТекущееПодразделение);
		...
		Если КлючВарианта = "Основной"  или КлючВарианта = "ВРазрезеВыходныхДней" Тогда
		...
		Если КлючВарианта = "Основной" Тогда
			...
			Для Каждого СтрокаСотрудник из Данные.Строки Цикл
				...
				Номер = 1;
				Для Каждого СтрокаДанных из СтрокаСотрудник.Строки Цикл
		...
		ИначеЕсли КлючВарианта = "ВРазрезеВыходныхДней" Тогда
			
			Макет = ПолучитьМакет("МакетВРазрезеВыходных");
			
			ОбластьШапка = Макет.ПолучитьОбласть("ОбластьШапка");
			ДокументРезультат.Вывести(ОбластьШапка);
			
			Для Каждого СтрокаСотрудник из Данные.Строки Цикл
				ТаблицаИтога = СоздатьПустуюТаблицу(Истина);
				Номер = 1;
				Для Каждого СтрокаДанных из СтрокаСотрудник.Строки Цикл
					ОбластьСотрудник = Макет.ПолучитьОбласть("ОбластьСотрудник");
					ОбластьСотрудник.Параметры.Заполнить(СтрокаДанных);
					...
					Номер = Номер + 1;
				КонецЦикла;
				ТаблицаИтога.Свернуть(,"ЧасовАванс,СуммаАванс,СуммаАвансНаРуки,ЧасовЗП,СуммаЗП,СуммаЗПНаРуки,ИТОГОЧасы,ИтогоСумма,ИтогоСуммаНаРуки,СуммаАвансВых,СуммаАвансНаРукиВых,СуммаЗПВых,СуммаЗПНаРукиВых,ИтогоСуммаВых,ИтогоСуммаНаРукиВых");
				ОбластьИтог = Макет.ПолучитьОбласть("ОбластьИтог");
				Если ТаблицаИтога.Количество()>0 Тогда
					ОбластьИтог.Параметры.Заполнить(ТаблицаИтога[0]);			
				КонецЕсли;
		...
		ИначеЕсли КлючВарианта = "Сокращенный" Тогда
			...
			Данные.Свернуть("Сотрудник","ЧасовАванс,СуммаАванс,СуммаАвансНаРуки,ЧасовЗП,СуммаЗП,СуммаЗПНаРуки,ИТОГОЧасы,ИтогоСумма,ИтогоСуммаНаРуки");
			Данные.Сортировать("Сотрудник возр");
