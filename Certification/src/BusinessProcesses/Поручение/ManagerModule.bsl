...
Функция ФормаВыполненияЗадачи(ЗадачаСсылка, ТочкаМаршрутаБизнесПроцесса) Экспорт
	...
	Результат.Вставить("ИмяФормы", "БизнесПроцесс.Поручение.Форма.Действие" + ТочкаМаршрутаБизнесПроцесса.Имя);
	Возврат Результат;
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Запрос.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Запрос.Текст, Метаданные.БизнесПроцессы.Поручение.ПолноеИмя());
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	БизнесПроцесс = Метаданные.БизнесПроцессы.Поручение;
	ИмяПроцедуры = "БизнесПроцессы.Поручение.ОбработатьДанныеДляПереходаНаНовуюВерсию";
	
	ПроблемныхОбъектов = 0;
	ОбъектовОбработано = 0;
	
	Задание = ОбновлениеИнформационнойБазы.ВыбратьСсылкиДляОбработки(Параметры.Очередь, "БизнесПроцесс.Поручение");
	...
	Параметры.ОбработкаЗавершена = ОбновлениеИнформационнойБазы.ОбработкаДанныхЗавершена(Параметры.Очередь, "БизнесПроцесс.Поручение");
...
Процедура ОбновитьНаборыЗначенийДоступаЗадания(Параметры, ЗаданиеСсылка) 
	...
	Попытка
		...
		ЭлементБлокировки = Блокировка.Добавить("БизнесПроцесс.Поручение");
