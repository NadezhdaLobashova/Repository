...
Процедура ВыбратьПрофильПользователя() Экспорт
	ИмяОбработки = ОбщегоНазначения.ПолучитьРольПользователя();	
	Попытка
		//ОткрытьФорму("Обработка.РабочийСтолРуководительПроектов.Форма", , , , ПолучитьОкна()[0]);// по опыту, первое окно - окно системы. В реальности, если вдруг случится не так - надо делать сложнее
	Исключение
