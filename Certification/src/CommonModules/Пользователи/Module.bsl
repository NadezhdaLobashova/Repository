...
Процедура ОпределитьТекущееПодразделение(Знач ИмяПараметра, УстановленныеПараметры) Экспорт
	...
	Запрос = Новый Запрос("ВЫБРАТЬ
	                      ...
	                      |ИЗ
	                      |	РегистрСведений.КадроваяИстория.СрезПоследних(&ДатаСреза, Сотрудник = &ТекущийПользователь) КАК КадроваяИсторияСрезПоследних");
	...
	Если НЕ Результат.Пустой() Тогда
		...
		Выборка.Следующий();
		ПараметрыСеанса.ТекущееПодразделение = Выборка.Подразделение;
	Иначе
...
Процедура ОпределитьПодразделенияВИерархии(УстановленныеПараметры) Экспорт
	
	УстановитьПривилегированныйРежим(Истина);
	
	//Если УстановленныеПараметры.Найти("ТекущийПользователь") = Неопределено Тогда
	//	Возврат
	//КонецЕсли;	
	
	Запрос = Новый Запрос("ВЫБРАТЬ
	                      |	РуководителиПодразделений.Подразделение
	                      |ИЗ
	                      |	РегистрСведений.РуководителиПодразделений КАК РуководителиПодразделений
	                      |ГДЕ
	                      |	РуководителиПодразделений.Сотрудник = &ТекущийПользователь");
	Запрос.УстановитьПараметр("ДатаСреза",ТекущаяДатаСеанса());
	Запрос.УстановитьПараметр("ТекущийПользователь",ПараметрыСеанса.ТекущийПользователь);
	тзПодразделенияРуководителя = Запрос.Выполнить().Выгрузить();
	МассивПодразделений = тзПодразделенияРуководителя.ВыгрузитьКолонку("Подразделение");	
	...
	Запрос = Новый Запрос("ВЫБРАТЬ
	                      ...
	                      |ГДЕ
	                      |	Подразделения.Ссылка В ИЕРАРХИИ(&ТекущееПодразделение)");
	Запрос.УстановитьПараметр("ТекущееПодразделение",МассивПодразделений);
	Результат = Запрос.Выполнить().Выгрузить();
	ПараметрыСеанса.РазрешенныеПодразделения = Новый ФиксированныйМассив(Результат.ВыгрузитьКолонку("Ссылка"));	
	
	УстановленныеПараметры.Добавить("РазрешенныеПодразделения");
	
КонецПроцедуры


Функция ТекущееПодразделениеПользователя() Экспорт
