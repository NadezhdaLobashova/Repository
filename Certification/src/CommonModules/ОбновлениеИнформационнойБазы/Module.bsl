...
Процедура ПерейтиНаВерсию_2_0_1_8() Экспорт
	...
	//Необходимо создать основной этап для всех проектов и заполнить в документах и регистрах
	
	//СозданиеЭтапов
	ПроверитьИСоздатьОсновнойЭтап();	
	
КонецПроцедуры	

&НаСервере
Процедура ПроверитьИСоздатьОсновнойЭтап()

	ЗаписьЖурналаРегистрации("Обновление ИБ",,,, "Начало");
	...
	Запрос.Текст = 
		...
		|	ЕСТЬNULL(ЭтапыПроектов.Ссылка, ЗНАЧЕНИЕ(Справочник.ЭтапыПроектов.ПустаяСсылка)) КАК Этап
		|ИЗ
		|	Справочник.Проекты КАК Проекты
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ЭтапыПроектов КАК ЭтапыПроектов
		|		ПО (ЭтапыПроектов.Владелец = Проекты.Ссылка)
		|			И (НЕ ЭтапыПроектов.ПометкаУдаления)
		|ГДЕ
		|	НЕ Проекты.ПометкаУдаления
		|	И ЕСТЬNULL(ЭтапыПроектов.Ссылка, ЗНАЧЕНИЕ(Справочник.ЭтапыПроектов.ПустаяСсылка)) = ЗНАЧЕНИЕ(Справочник.ЭтапыПроектов.ПустаяСсылка)";
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		Новыйэтап.Владелец = ВыборкаДетальныеЗаписи.Ссылка;
		НовыйЭтап.Наименование = ВыборкаДетальныеЗаписи.Наименование + " (Основной этап)";
		Попытка
			НовыйЭтап.Записать();
		Исключение
			ЗаписьЖурналаРегистрации("Обновление ИБ",,,,"Не удалось создать этап для проекта " + ВыборкаДетальныеЗаписи.Наименование + " по причине: " + ОписаниеОшибки());
		КонецПопытки;
	КонецЦикла;	
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		ЗаданиеОб = ВыборкаДетальныеЗаписи.Ссылка.ПолучитьОбъект();
		ЗаданиеОб.ЭтапПроекта = УРВСервер.НайтиОсновнойЭтапДляПодстановкиВДокументы(ВыборкаДетальныеЗаписи.Проект);
		Попытка
		    Если ВыборкаДетальныеЗаписи.Ссылка.Проведен Тогда
				ЗаданиеОб.Записать(РежимЗаписиДокумента.Проведение);
			Иначе
				ЗаданиеОб.Записать(РежимЗаписиДокумента.Запись);
			КонецЕсли;
		
		Исключение
			ЗаписьЖурналаРегистрации("Обновление ИБ",,,,"Не удалось провести документ " + ВыборкаДетальныеЗаписи.Ссылка + " по причине: " + ОписаниеОшибки());
		КонецПопытки;
	КонецЦикла;
	...
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	ОперативныйПрогноз.Ссылка КАК Ссылка,
		|	ОперативныйПрогноз.Проект КАК Проект
		|ИЗ
		|	Документ.ОперативныйПрогноз КАК ОперативныйПрогноз
		|ГДЕ
		|	НЕ ОперативныйПрогноз.ПометкаУдаления";
	...
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	ПланПроекта.Ссылка КАК Ссылка,
		|	ПланПроекта.Проект КАК Проект
		|ИЗ
		|	Документ.ПланПроекта КАК ПланПроекта
		|ГДЕ
		|	НЕ ПланПроекта.ПометкаУдаления";
	...
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	ПрогнозПроекта.Ссылка КАК Ссылка,
		|	ПрогнозПроекта.Проект КАК Проект
		|ИЗ
		|	Документ.ПрогнозПроекта КАК ПрогнозПроекта
		|ГДЕ
		|	НЕ ПрогнозПроекта.ПометкаУдаления";
	...
	//ФактическиеПоказателиПроекта
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	ФактическиеПоказателиПроекта.Ссылка КАК Ссылка,
		|	ФактическиеПоказателиПроекта.Проект КАК Проект
		|ИЗ
		|	Документ.ФактическиеПоказателиПроекта КАК ФактическиеПоказателиПроекта
		|ГДЕ
		|	НЕ ФактическиеПоказателиПроекта.ПометкаУдаления";
	...
	//УчетФактическихДанных
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	УчетФактическихДанных.Ссылка КАК Ссылка
		|ИЗ
		|	Документ.УчетФактическихДанных КАК УчетФактическихДанных
		|ГДЕ
		|	НЕ УчетФактическихДанных.ПометкаУдаления";
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		ЗаданиеОб = ВыборкаДетальныеЗаписи.Ссылка.ПолучитьОбъект();
		Для Каждого Стр Из ЗаданиеОб.Время Цикл
			Стр.ЭтапПроекта = УРВСервер.НайтиОсновнойЭтапДляПодстановкиВДокументы(Стр.Проект);
		КонецЦикла;
	...
	Запрос.Текст = 
		...
		|ГДЕ
		|	НЕ ЛистУчетаРабочегоВремени.ПометкаУдаления";
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		ЗаданиеОб = ВыборкаДетальныеЗаписи.Ссылка.ПолучитьОбъект();
		Для Каждого Стр Из ЗаданиеОб.Работы Цикл
			Стр.ЭтапПроекта = УРВСервер.НайтиОсновнойЭтапДляПодстановкиВДокументы(Стр.Проект);
		КонецЦикла;
	...
	ЗаписьЖурналаРегистрации("Обновление ИБ",,,, "Окончание");
	
КонецПроцедуры

Процедура ПерейтиНаВерсию_2_0_1_9() Экспорт
	
	ЗаписьЖурналаРегистрации("Обновление ИБ",,,, "Начало");
	
	НаборЗаписей = РегистрыСведений.НастройкиИнтеграции.СоздатьНаборЗаписей();
	НаборЗаписей.Отбор.Интеграция.Установить(Перечисления.Интеграции.ПолучениеДанныхИзОБД);
	
	НоваяЗапись = НаборЗаписей.Добавить();
	НоваяЗапись.Интеграция = Перечисления.Интеграции.ПолучениеДанныхИзОБД;
	НоваяЗапись.АдресПодключения = Константы.ОБД_Порт.Получить();
	НоваяЗапись.Пользователь = Константы.ОБД_Пользователь.Получить();
	НоваяЗапись.Пароль = Константы.ОБД_Пароль.Получить();
...
Процедура ПерейтиНаВерсию_2_0_1_10() Экспорт
	...
	Запрос.Текст = 
		...
		|	ЭтапыПроектов.Владелец.RedmineID КАК RedmineID
		|ИЗ
		|	Справочник.ЭтапыПроектов КАК ЭтапыПроектов
		|ГДЕ
		|	ЭтапыПроектов.Владелец.RedmineID <> """"";
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		ЭтапОбъект.RedmineID = ВыборкаДетальныеЗаписи.RedmineID;
		ЭтапОбъект.Записать();
	КонецЦикла;
...
Процедура ПерейтиНаВерсию_1_0_14_4() Экспорт
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		Для Каждого Стр Из ЗаданиеОб.Время Цикл
			Если Стр.Выходной Тогда
				Стр.ВидРабочегоВремени = Справочники.ВидыРабочегоВремени.РаботаВВыходныеДни;
			Иначе	
				Стр.ВидРабочегоВремени = Справочники.ВидыРабочегоВремени.РаботаВОсновноеВремя;
			КонецЕсли; 
		КонецЦикла;
...
Процедура ПерейтиНаВерсию_1_0_14_4_ДоработкаРегион() Экспорт
	
	ЗаписьЖурналаРегистрации("Обновление ИБ",,,, "Доработка Регион. Начало");
	
	//Установка коэффициента для Новосибирска
	ОбРегион = Справочники.Регионы.Новосибирск.ПолучитьОбъект();
	ОбРегион.РайонныйКоэффициент = 1.2;
	ОбРегион.Записать();
	...
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	ИзменениеСостоянияСотрудника.Ссылка КАК Ссылка
		|ИЗ
		|	Документ.ИзменениеСостоянияСотрудника КАК ИзменениеСостоянияСотрудника
		|ГДЕ
		|	НЕ ИзменениеСостоянияСотрудника.ПометкаУдаления";
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		ЗаданиеОб = ВыборкаДетальныеЗаписи.Ссылка.ПолучитьОбъект();
		ЗаданиеОб.Регион = ОбРегион.Ссылка;
		Попытка
		    Если ВыборкаДетальныеЗаписи.Ссылка.Проведен Тогда
				ЗаданиеОб.Записать(РежимЗаписиДокумента.Проведение);
			Иначе
	...
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	УстановкаСтавок.Ссылка КАК Ссылка
		|ИЗ
		|	Документ.УстановкаСтавок КАК УстановкаСтавок
		|ГДЕ
		|	НЕ УстановкаСтавок.ПометкаУдаления";
	...
	ЗаписьЖурналаРегистрации("Обновление ИБ",,,, "Доработка Регион. Окончание");
