...
Процедура СоздатьПисьмо(Сообщение, УчетнаяЗапись) Экспорт
	...
	Попытка
		...
		Письмо.Дата			        	= ТекущаяУниверсальнаяДата();
...
// Процедура обновления ИБ для версии БСП 1.2.1.4.
// Преобразовывает начало описания документов "Телефонный звонок" в
// реквизит Тема.
//
...
Процедура РассчитатьРассмотреноПоПапкам(ТаблицаПапок) Экспорт
	...
	//Расчетные регистры не применяются. Процедура выполняться не будет
	Возврат;
...
Функция ТекстЗапросаСпискаВзаимодействий(ЗначениеОтбора = Неопределено) Экспорт
	...
	|ВЫБРАТЬ
    |   ВЫБОР
    |       КОГДА ЖурналДокументовВзаимодействия.Ссылка ССЫЛКА Документ.КомментарийПользователя И ЖурналДокументовВзаимодействия.Входящий ТОГДА 0
    |       КОГДА ЖурналДокументовВзаимодействия.Ссылка ССЫЛКА Документ.КомментарийПользователя И НЕ ЖурналДокументовВзаимодействия.Входящий ТОГДА 1
    |       КОГДА ЖурналДокументовВзаимодействия.Ссылка ССЫЛКА Документ.ТелефонныйЗвонок И ЖурналДокументовВзаимодействия.Входящий ТОГДА 2
    |       КОГДА ЖурналДокументовВзаимодействия.Ссылка ССЫЛКА Документ.ТелефонныйЗвонок И НЕ ЖурналДокументовВзаимодействия.Входящий ТОГДА 3
    |       КОГДА ЖурналДокументовВзаимодействия.Ссылка ССЫЛКА Документ.ЭлектронноеПисьмоИсходящее И НЕ ЖурналДокументовВзаимодействия.СтатусИсходящегоПисьма = &Черновик ТОГДА 4
    |       КОГДА ЖурналДокументовВзаимодействия.Ссылка ССЫЛКА Документ.ЭлектронноеПисьмоВходящее ТОГДА 5
    |       КОГДА ЖурналДокументовВзаимодействия.Ссылка ССЫЛКА Документ.ЭлектронноеПисьмоИсходящее И ЖурналДокументовВзаимодействия.СтатусИсходящегоПисьма = &Черновик ТОГДА 6
    |   КОНЕЦ + ВЫБОР
    |   КОГДА ЖурналДокументовВзаимодействия.ПометкаУдаления ТОГДА 7
	...
	|	ЕСТЬNULL(ДОБАВИТЬКДАТЕ(ЖурналДокументовВзаимодействия.ПолученоОтправлено, СЕКУНДА, &СекундДоМестногоВремени), ДАТАВРЕМЯ(1, 1, 1)) КАК РассмотретьПосле,
...
Процедура ПриЗаписиДокумента(ДокументОбъект) Экспорт
	...
	Если ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.ТелефонныйЗвонок") Тогда
...
Процедура ЗаполнитьКонтактыМассиваВзаимодействий(МассивВзаимодействий, РассчитыватьРассмотрено = Ложь) Экспорт
	...
	Запрос.Текст = "
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|    ТелефонныйЗвонок.Ссылка КАК Взаимодействие,
	|    ТелефонныйЗвонок.АбонентКонтакт КАК Контакт,
	|    ТелефонныйЗвонок.АбонентПредставление КАК ПредставлениеКонтакта
	...
	|ГДЕ
	|    ТелефонныйЗвонок.Ссылка В(&МассивВзаимодействий)
	|
	|ОБЪЕДИНИТЬ
	|
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
